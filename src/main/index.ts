...
function createWindow(): void {
  ...
  if (is.dev && process.env['ELECTRON_RENDERER_URL']) {
    ...
    secondaryWindow.loadURL(`${process.env['ELECTRON_RENDERER_URL']}/secondaryWindow.html`);
  } else {
    mainWindow.loadFile(path.join(__dirname, `../renderer/index.html`));
    secondaryWindow.loadFile(path.join(__dirname, `../renderer/secondaryWindow.html`));
  }
  ...
  for (const pack of localPacks) {
    ...
    if (index === -1) {
      _packs.push({
        ...
        local: getConfig().packs.find((p) => p.id === pack.id) as {
          id: string;
          path: string;
          ram: number;
        }
